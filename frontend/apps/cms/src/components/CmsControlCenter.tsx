...
function initialNews(item: R | null): NewsForm {
  ...
  return {
    ...
    fact_check_status: factCheckStatus(news.fact_check_status),
    legal_review_status: legalReviewStatus(news.legal_review_status),
    editorial_priority: editorialPriority(news.editorial_priority),
    sensitivity: sensitivity(news.sensitivity)
  };
}
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-4 md:px-6 lg:px-8">
        ...
        {workspace === 'news' ? (
          <section className="mt-6 grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
            <aside className="space-y-3">
              <div className={card}>
                <div className="flex items-center justify-between gap-3"><div><h2 className="font-bold">Editorial queue</h2><p className="text-xs text-slate-500">Review sampai retract.</p></div><SelectField value={newsStatus} onChange={e => setNewsStatus(e.target.value)} className="rounded-xl border border-[color:var(--color-border)] px-2.5 py-2 text-xs"><option value="pending_review">Review</option><option value="needs_revision">Revisi</option><option value="published">Published</option><option value="rejected">Rejected</option><option value="retracted">Retracted</option><option value="all">Semua</option></SelectField></div>
              </div>
              ...
              {selectedNews ? <>
                ...
                <div className="grid gap-6 2xl:grid-cols-[1.15fr_0.85fr]">
                  ...
                  <div className="space-y-4">
                    ...
                    <section className={card}><h3 className="font-bold">Publish controls</h3><div className="grid gap-3 md:grid-cols-2"><label><span className="text-xs font-semibold text-slate-500">Publish at</span><input type="datetime-local" value={newsForm.publish_at} onChange={e => setNewsForm(p => ({ ...p, publish_at: e.target.value }))} className={input} /></label><label><span className="text-xs font-semibold text-slate-500">Fact check</span><SelectField value={newsForm.fact_check_status} onChange={e => setNewsForm(p => ({ ...p, fact_check_status: factCheckStatus(e.target.value) }))} className={input}><option value="pending">Pending</option><option value="verified">Verified</option><option value="not_required">Tidak perlu</option></SelectField></label><label><span className="text-xs font-semibold text-slate-500">Legal</span><SelectField value={newsForm.legal_review_status} onChange={e => setNewsForm(p => ({ ...p, legal_review_status: legalReviewStatus(e.target.value) }))} className={input}><option value="not_required">Tidak perlu</option><option value="pending">Pending</option><option value="approved">Approved</option></SelectField></label><label><span className="text-xs font-semibold text-slate-500">Priority</span><SelectField value={newsForm.editorial_priority} onChange={e => setNewsForm(p => ({ ...p, editorial_priority: editorialPriority(e.target.value) }))} className={input}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></SelectField></label><label className="md:col-span-2"><span className="text-xs font-semibold text-slate-500">Sensitivity</span><SelectField value={newsForm.sensitivity} onChange={e => setNewsForm(p => ({ ...p, sensitivity: sensitivity(e.target.value) }))} className={input}><option value="normal">Normal</option><option value="high">High</option></SelectField></label></div><label className="mt-3 block"><span className="text-xs font-semibold text-slate-500">Catatan editor</span><textarea value={newsForm.note} onChange={e => setNewsForm(p => ({ ...p, note: e.target.value }))} rows={4} className={input} /></label></section>
                  </div>
                </div>
        ...
        {workspace === 'moderation' ? (
          <section className="mt-6 space-y-5">
            <div className={card}><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-bold">Moderation Center</h2><p className="text-sm text-slate-500">Reports, severity, visibility, dan audit.</p></div><SelectField value={moderationStatus} onChange={e => setModerationStatus(e.target.value)} className="rounded-xl border border-[color:var(--color-border)] px-3 py-2 text-sm"><option value="open">Open</option><option value="reviewing">Reviewing</option><option value="escalated">Escalated</option><option value="resolved">Resolved</option><option value="appealed">Appealed</option></SelectField></div></div>
