...
const GOOGLE_OAUTH_STATE_COOKIE = 'cms_google_oauth_state';
const ALLOWED_ROLES = new Set(["content_admin","admin","super_admin"]);
...
function redirectUri(req: NextRequest): string {
  return process.env.CMS_GOOGLE_REDIRECT_URI || requestOrigin(req) + '/api/auth/google/callback';
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!roles.some(role => ALLOWED_ROLES.has(role.toLowerCase()))) {
      await revokeSession(auth.access_token);
      return failure(req, 'google_account_not_authorized_for_cms');
    }
