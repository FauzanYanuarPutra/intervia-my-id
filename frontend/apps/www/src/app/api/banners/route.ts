...
export async function GET(req: NextRequest) {
  ...
  try {
    const backendRes = await fetch(
      `${marketplaceBase}/v1/banners${query ? `?${query}` : ''}`,
      {
        method: 'GET',
    ...
    if (!backendRes.ok) {
      console.error('[api/banners] backend error:', backendRes.status, data);
      return NextResponse.json(
        {
    ...
    console.error('[api/banners] marketplace unreachable:', message);
    return NextResponse.json(
