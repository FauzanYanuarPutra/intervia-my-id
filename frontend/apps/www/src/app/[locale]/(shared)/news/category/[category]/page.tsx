...
export default async function NewsCategoryPage({ params, searchParams }: Props) {
  ...
  return <main className="page-shell page-rhythm pb-12 pt-6">
    ...
    {items.length ? <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map(article => <Link key={article.id} href={buildNewsPath(article.slug)} className="group flex min-h-[240px] flex-col overflow-hidden rounded-[24px] border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-sm dark:border-white/10 dark:bg-slate-900">
        ...
        <div className="flex min-h-0 flex-1 flex-col p-4 sm:p-5">
          ...
          <h2 className="mt-2 text-xl font-bold tracking-[-0.04em] text-slate-950 group-hover:text-emerald-800 dark:text-white dark:group-hover:text-emerald-200">{article.title}</h2>
