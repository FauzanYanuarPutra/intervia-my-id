...
export default async function NewsIndexPage({ params, searchParams }: PageProps) {
  ...
  return (
    <main className="page-shell page-rhythm pb-12 pt-6">
      ...
      {rest.length > 0 ? (
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]"><div><div className="mb-4 border-b-2 border-slate-950 pb-2 dark:border-white"><h2 className="text-xl font-black text-slate-950 dark:text-white">{isId ? 'Berita terbaru' : 'Latest news'}</h2></div><div className="divide-y divide-slate-200 dark:divide-white/10">{rest.map(article => <Link key={article.id} href={buildNewsPath(article.slug)} className="group grid gap-4 py-5 sm:grid-cols-[180px_minmax(0,1fr)]"><div className="aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-slate-800">{article.coverImage ? <img
