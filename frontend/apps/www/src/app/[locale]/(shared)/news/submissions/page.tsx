...
export default async function MyNewsSubmissionsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="page-shell page-rhythm pb-12 pt-6">
    <Link href="/manage/content" className="inline-flex min-h-9 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200"><ArrowLeft className="h-3.5 w-3.5"/>Kembali ke Konten</Link>
