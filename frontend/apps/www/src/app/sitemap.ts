...
type SitemapContentItem = { id?: string | null; slug?: string | null; title?: string | null; type?: string | null; content_type?: string | null; status?: string | null; content_status?: string | null; created_at?: string | null; updated_at?: string | null };
...
function safeDate(value: unknown): Date | undefined {
  const raw = readText(value); if (!raw) return undefined;
  const date = new Date(raw); return Number.isNaN(date.getTime()) ? undefined : date;
}
...
async function fetchContentSitemapItems(): Promise<SitemapContentItem[]> {
  const offsets = Array.from({ length: CONTENT_SITEMAP_MAX_ITEMS / CONTENT_SITEMAP_LIMIT }, (_, index) => index * CONTENT_SITEMAP_LIMIT);
  const pages = await Promise.all(offsets.map(async offset => {
    ...
    const params = new URLSearchParams({ limit: String(CONTENT_SITEMAP_LIMIT), offset: String(offset), content_status: 'active' });
    try {
      const res = await fetch(`${marketplaceBase}/v1/content?${params}`, { signal: controller.signal, next: { revalidate } });
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const routes: Array<{ path: string; priority: number; changeFrequency: MetadataRoute.Sitemap[number]['changeFrequency'] }> = [
    ...
    ...LAJUKAN_EXPLORE_CATEGORIES.map(category => ({ path: `/explore/${category.slug}`, priority: 0.9, changeFrequency: 'daily' as const })),
  ...
  routes.forEach(route => locales.forEach(lang => sitemapEntries.push({
    url: `${baseUrl}/${lang}${route.path}`, changeFrequency: route.changeFrequency, priority: route.priority,
    alternates: { languages: { id: `${baseUrl}/id${route.path}`, en: `${baseUrl}/en${route.path}`, 'x-default': `${baseUrl}/id${route.path}` } },
  })));
  BLOG_ARTICLES.filter(isBlogArticleIndexable).forEach(article => {
    const lastModified = safeDate(article.updatedAt);
    locales.forEach(lang => sitemapEntries.push({
      url: `${baseUrl}/${lang}/blog/${article.slug}`, lastModified, changeFrequency: 'monthly', priority: 0.76,
      alternates: { languages: { id: `${baseUrl}/id/blog/${article.slug}`, en: `${baseUrl}/en/blog/${article.slug}`, 'x-default': `${baseUrl}/id/blog/${article.slug}` } },
    }));
  });
  const seenContentPaths = new Set<string>();
  const contentItems = await fetchContentSitemapItems();
  contentItems.forEach(item => {
    const id = readText(item.id); const title = readText(item.title) || readText(item.slug) || 'listing';
    const path = buildContentHref(id, title, readText(item.slug) || undefined);
    if (!path || seenContentPaths.has(path)) return; seenContentPaths.add(path);
    const lastModified = safeDate(item.updated_at || item.created_at);
    locales.forEach(lang => sitemapEntries.push({
      url: `${baseUrl}/${lang}${path}`, lastModified, changeFrequency: 'daily', priority: contentPriority(item),
      alternates: { languages: { id: `${baseUrl}/id${path}`, en: `${baseUrl}/en${path}`, 'x-default': `${baseUrl}/id${path}` } },
    }));
  });
