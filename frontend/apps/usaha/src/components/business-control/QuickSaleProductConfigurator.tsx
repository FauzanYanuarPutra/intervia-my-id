...
export function QuickSaleProductConfigurator({
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-white" aria-label={`${editing ? 'Edit' : 'Atur'} ${productName}`}>
      <header className="shrink-0 border-b border-portal-line px-4 py-3.5 sm:px-5">
        <div className="flex items-start gap-3">
          <button type="button" aria-label="Kembali ke pesanan" className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-portal-line text-portal-soft hover:bg-[#f2f4f1] hover:text-portal-ink" onClick={close}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-xs leading-5 text-portal-soft">{editing ? 'Ubah racikan tanpa membuat produk baru di katalog.' : 'Pilih ukuran, topping, atau racikan sesuai pesanan.'}</p>
          </div>
        </div>
      </header>
