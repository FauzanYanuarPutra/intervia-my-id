...
export function PortalShell({
  ...
  const businesses =
    activeBusiness && !availableBusinesses.some(item => item.id === activeBusiness.id)
      ? [activeBusiness, ...availableBusinesses]
      : availableBusinesses;

  return (
    <div className="min-h-screen bg-[#f7f8f6] text-portal-ink">
      <a href="#portal-content" className="portal-skip-link">
        Langsung ke konten
      </a>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[240px] flex-col border-r border-portal-line bg-white px-3 py-3 lg:flex">
        <Link
          href="/"
          className="flex min-h-11 items-center gap-2.5 rounded-xl px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-portal-forest/20"
        >
          <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-portal-forest text-white">
            <Store className="h-4 w-4" />
          </span>
          <span>
            <span className="block text-[11px] font-semibold text-portal-soft">Lajukan</span>
        ...
        <div className="mt-3 border-t border-portal-line pt-3">
          <Link
            href="/access"
            className="mb-1 flex min-h-10 items-center gap-3 rounded-xl px-2 text-sm font-semibold text-portal-soft transition hover:bg-portal-mist hover:text-portal-forest"
          >
          ...
          {activeBusiness?.permissions.includes('manageSecurity') ? (
            <Link
              href={`/security?business=${activeBusiness.id}`}
              className="mb-1 flex min-h-10 items-center gap-3 rounded-xl px-2 text-sm font-semibold text-portal-soft transition hover:bg-portal-mist hover:text-portal-forest"
            >
              <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-portal-mist text-portal-forest">
          ...
          <div className="flex items-center gap-3 rounded-xl px-2 py-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[11px] font-semibold text-portal-soft">Akun</p>
      ...
      <div className="min-h-screen lg:pl-[240px]">
        <header className="sticky top-0 z-30 border-b border-portal-line bg-white/95 backdrop-blur-xl">
          <div className="mx-auto flex min-h-14 w-full max-w-[1440px] items-center justify-between gap-3 px-4 sm:px-5 lg:px-6">
            <div className="flex min-w-0 items-center gap-3 lg:hidden">
              <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] bg-portal-forest text-white">
                <Store className="h-5 w-5" />
              </span>
              <div className="min-w-0">
                <p className="text-[11px] font-semibold text-portal-soft">
                  {accountPage
                    ? 'Akun Lajukan'
                    : activeBusiness
                      ? activeBusiness.name
            ...
            <div className="flex shrink-0 items-center gap-2">
              {!accountPage && status ? (
                <StatusBadge tone={activeBusiness?.isOpen ? 'success' : 'neutral'}>
                  {status.label}
                </StatusBadge>
              ) : null}
              {viewerName ? <InvitationIndicator /> : null}
              <Link href="/businesses/new" className="portal-button-secondary hidden sm:inline-flex">
        ...
        <main
          id="portal-content"
          tabIndex={-1}
          className="mx-auto w-full max-w-[1440px] px-3 pb-24 pt-4 outline-none sm:px-5 lg:px-6 lg:pb-8"
        >
