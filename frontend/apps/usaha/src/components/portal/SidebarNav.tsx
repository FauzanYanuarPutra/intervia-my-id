...
export function SidebarNav({ business, currentSection }: SidebarNavProps) {
  ...
  return (
    <nav className="mt-5 space-y-5" aria-label="Navigasi usaha">
      {groups.map(group => (
        <div key={group.label}>
          <p className="px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-white/40">{group.label}</p>
            ...
            {group.items.map(item => {
              ...
              return (
                <Link
                  ...
                  className={`flex min-h-10 items-center gap-3 rounded-[13px] px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/25 ${active ? 'bg-white text-portal-forest shadow-sm' : 'text-white/70 hover:bg-white/[0.08] hover:text-white'}`}
                >
