...
export function MobileNav({ business, currentSection }: MobileNavProps) {
  ...
  const more = [...workItems, ...settingsItems];
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-portal-line bg-white/95 px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-10px_30px_-20px_rgba(15,23,42,.3)] backdrop-blur lg:hidden" aria-label="Navigasi usaha mobile">
      <div className="mx-auto grid max-w-md grid-cols-4 gap-1">
        ...
        <details className="group relative">
          <summary className={`flex min-h-12 cursor-pointer list-none flex-col items-center justify-center gap-1 rounded-xl px-2 text-[10px] font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-portal-forest/20 ${more.some(item => item.id === currentSection) ? 'bg-portal-mist text-portal-forest' : 'text-portal-soft hover:bg-portal-mist/70'}`}>
          ...
          <div className="absolute bottom-[calc(100%+.65rem)] right-0 max-h-[70vh] w-[290px] overflow-y-auto rounded-[20px] border border-portal-line bg-white p-2 shadow-[0_24px_70px_-24px_rgba(15,23,42,.5)]">
            {workItems.length ? <><p className="px-2 pb-1 pt-1 text-[10px] font-bold uppercase tracking-[.12em] text-portal-soft/70">Kerja utama</p>{workItems.map(menuLink)}</> : null}
            {settingsItems.length ? <><p className="mt-2 border-t border-portal-line px-2 pb-1 pt-3 text-[10px] font-bold uppercase tracking-[.12em] text-portal-soft/70">Pengaturan usaha</p>{settingsItems.map(menuLink)}</> : null}
          </div>
        </details>
      </div>
    </nav>
