...
export function MobileNav({ business, currentSection }: MobileNavProps) {
  ...
  const management = more.filter(item => ['products', 'reports', 'channels', 'buyerPage', 'work'].includes(item.id));
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 border-b border-portal-line px-4 pb-3 pt-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-portal-soft">Pekerjaan, produk, laporan, kanal jual, dan pengaturan usaha.</p>
          </div>
          <button
            type="button"
