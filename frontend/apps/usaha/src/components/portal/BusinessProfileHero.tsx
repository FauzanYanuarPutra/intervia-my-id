...
type BusinessProfileHeroProps = { business: BusinessRecord };
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-portal-line bg-white shadow-[0_18px_48px_-40px_rgba(15,23,42,.35)]">
      <div className="relative h-28 overflow-hidden bg-[linear-gradient(135deg,#dff5e8_0%,#f7faf7_55%,#e9f3ed_100%)] sm:h-36 lg:h-40">
        {business.bannerUrl ? <Image src={business.bannerUrl} alt="" fill unoptimized sizes="(max-width: 640px) 100vw, 1120px" className="object-cover" /> : null}
        <div className="absolute inset-0 bg-gradient-to-b from-black/5 via-transparent to-black/25" />
        <div className="absolute right-3 top-3"><StatusBadge tone={business.isOpen ? 'success' : 'neutral'}>{status.label}</StatusBadge></div>
      </div>
      ...
      <div className="px-3 pb-4 sm:px-5 sm:pb-5">
        <div className="-mt-10 flex min-w-0 items-end gap-3 sm:-mt-12 sm:gap-4">
          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-full border-[4px] border-white bg-[#f3f6f3] shadow-lg sm:h-24 sm:w-24">
            {business.logoUrl ? (
              <Image src={business.logoUrl} alt={business.name} fill unoptimized sizes="96px" className="object-cover" />
            ) : (
              <div className="grid h-full w-full place-items-center text-2xl font-black text-portal-forest">{business.name.slice(0, 1).toUpperCase()}</div>
            )}
          </div>

          <div className="min-w-0 flex-1 pb-1">
            <div className="flex min-w-0 items-center gap-1.5">
              <h1 className="min-w-0 truncate text-xl font-black tracking-[-0.03em] text-portal-ink sm:text-2xl">{business.name}</h1>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-portal-soft">
        ...
        <div className="mt-4 grid grid-cols-3 divide-x divide-portal-line rounded-2xl border border-portal-line py-2.5 sm:max-w-[620px]">
          <div className="px-2 text-center"><p className="text-base font-black text-portal-ink sm:text-lg">{business.products.length}</p><p className="mt-0.5 text-[10px] font-semibold text-portal-soft sm:text-xs">Produk</p></div>
          <div className="px-2 text-center"><p className="text-base font-black text-portal-ink sm:text-lg">{business.activeOrders}</p><p className="mt-0.5 text-[10px] font-semibold text-portal-soft sm:text-xs">Pesanan</p></div>
          <div className="px-2 text-center"><p className="text-base font-black text-portal-ink sm:text-lg">{stockAttention}</p><p className="mt-0.5 text-[10px] font-semibold text-portal-soft sm:text-xs">Perlu cek</p></div>
        </div>

        <div className="mt-3 grid grid-cols-2 gap-2 sm:flex">
          <Link href={canManageInfo ? `/businesses/${business.id}/info` : `/businesses/${business.id}/buyer-page`} className="portal-button-secondary"><PencilLine className="h-4 w-4" />{canManageInfo ? 'Edit profil' : 'Lihat profil'}</Link>
          {canSell ? <Link href={`/businesses/${business.id}/orders`} className="portal-button-primary"><ShoppingBag className="h-4 w-4" /> Jual</Link> : null}
        </div>
