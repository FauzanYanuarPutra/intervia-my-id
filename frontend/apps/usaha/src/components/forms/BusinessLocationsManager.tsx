...
export function BusinessLocationsManager({ businessId, businessName, initialLocations }: Props) {
  ...
  async function save(next: BusinessLocation[], reason = changeReason.trim()) {
    ...
    if (!next.length) {
      setError('Usaha harus memiliki minimal satu lokasi.');
      return false;
    }
  ...
  async function confirmDelete() {
    ...
    const ok = await save(locations.filter(item => item.id !== removed.id), changeReason);
  ...
  const point: LatLng | null =
    editing?.latitude != null && editing?.longitude != null
      ...
      : null;
  const editingExists = editing ? locations.some(item => item.id === editing.id) : false;
  const canDeletePersisted = editingExists && locations.length > 1;

  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-portal-ink">{locations.length} lokasi</p>
        ...
        {locations.map(location => (
          ...
          >
            <div className="flex items-start gap-3">
              <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-portal-forest/10 text-portal-forest">
                <MapPin className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-bold text-portal-ink">{location.name || 'Lokasi usaha'}</p>
                  {location.isPrimary ? (
                    <span className="rounded-full bg-portal-sand px-2 py-0.5 text-[10px] font-bold text-portal-forest">Utama</span>
                  ) : null}
                </div>
                <p className="mt-1 text-xs leading-5 text-portal-soft">
                  {[location.address, location.city].filter(Boolean).join(', ') || 'Alamat belum lengkap'}
                </p>
              </div>
            </div>
          </button>
      ...
      {editing ? (
        <section className="rounded-[24px] border border-portal-line bg-white p-4 sm:p-5">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-black text-portal-ink">{editingExists ? 'Edit lokasi' : 'Lokasi baru'}</p>
              <p className="mt-0.5 text-xs text-portal-soft">
                Simpan nama dan pin peta yang benar agar outlet mudah dikenali.
              </p>
            </div>
            <button
              type="button"
          ...
          <label className="mt-4 grid gap-1.5 text-sm font-semibold">
            ...
            <input
              ...
              maxLength={500}
              placeholder="Contoh: pindah outlet / koreksi alamat"
              className="portal-input"
            />
          </label>
          ...
          <div className="mt-4 flex flex-wrap justify-between gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                disabled={saving || !editing.name.trim() || editing.latitude === null || editing.longitude === null}
                className="portal-button-primary"
                onClick={() => {
                  const normalized = editing.isPrimary
                    ? locations.map(item => ({ ...item, isPrimary: false }))
                    : locations;
                  const exists = normalized.some(item => item.id === editing.id);
                  void save(
                    exists
                      ? normalized.map(item => item.id === editing.id ? editing : item)
                      : [...normalized, editing],
                  );
                }}
              >
                <Save className="h-4 w-4" /> {saving ? 'Menyimpan...' : 'Simpan lokasi'}
              </button>
            </div>
          </div>
        </section>
      ...
      <SensitiveActionConfirm
        ...
        title="Hapus lokasi?"
        description={`${pendingDelete?.name || 'Lokasi ini'} akan dihapus dari daftar outlet. Lokasi dan transaksi lain tidak berubah.`}
