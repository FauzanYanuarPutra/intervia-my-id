...
export default async function HomePage({
  ...
  const [financeCore, workItems] = await Promise.all([
    canViewFinance
      ? getFinanceCoreSummary(business.id).catch(() => null)
  ...
  return (
    <PortalShell activeBusiness={business} availableBusinesses={state.businesses} viewerName={viewerName} currentSection="home">
      ...
      <section className="merchant-surface-bordered overflow-hidden">
        <div className="border-b border-portal-line px-4 py-3.5 sm:px-5">
          <p className="text-[10px] font-black uppercase tracking-[0.1em] text-portal-soft">Kondisi usaha</p>
          <h2 className="mt-1 text-base font-black text-portal-ink">Yang perlu kamu tahu sekarang</h2>
        </div>
        <div className="grid gap-px bg-portal-line sm:grid-cols-2 lg:grid-cols-4">
          {canViewFinance ? (() => {
            const cash = financeCore?.accounts.find(account => account.account_key === 'cash')?.balance ?? 0;
            const ewallet = financeCore?.accounts.find(account => account.account_key === 'ewallet')?.balance ?? 0;
            return <>
              <div className="bg-white p-4"><p className="text-xs text-portal-soft">Kas</p><p className="mt-1 text-lg font-black text-portal-ink">{money.format(cash)}</p></div>
              <div className="bg-white p-4"><p className="text-xs text-portal-soft">E-wallet</p><p className="mt-1 text-lg font-black text-portal-ink">{money.format(ewallet)}</p></div>
            </>;
          })() : null}
      ...
      {workItems.some(item => !['done', 'cancelled'].includes(item.status)) ? (
        <section>
          <div className="mb-2.5 flex items-end justify-between gap-3">
            <div><h2 className="font-black text-portal-ink">Pekerjaan yang berjalan</h2><p className="mt-0.5 text-xs text-portal-soft">Supaya kondisi usaha langsung berubah jadi tindakan.</p></div>
            <Link href={`/businesses/${business.id}/work`} className="text-xs font-black text-portal-forest">Lihat semua</Link>
          </div>
          <div className="merchant-list border border-portal-line/80">
            {workItems.filter(item => !['done','cancelled'].includes(item.status)).slice(0, 3).map(item => (
              <Link key={item.id} href={`/businesses/${business.id}/work`} className="merchant-action-row">
                <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-portal-ink">{item.title}</p><p className="mt-0.5 text-[11px] text-portal-soft">{item.assignee_user_id ? 'Sudah ditugaskan' : 'Belum ditugaskan'}</p></div>
                <span className="text-xs font-black text-portal-forest">Kerjakan</span>
              </Link>
            ))}
          </div>
        </section>
