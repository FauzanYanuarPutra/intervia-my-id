...
async function fetchJson(url: URL): Promise<{
  status: number;
  body: unknown;
}> {
...
> {
  try {
    const url = new URL(
      `/users/public/${encodeURIComponent(id)}`,
      IDENTITY_URL,
    );
    ...
    return profileId(profile)
      ? { status: 'found', profile: profile as ProfileRecord }
      : { status: 'unavailable' };
  } catch {
    return { status: 'unavailable' };
  }
...
async function discoverProfiles(
  query?: string,
  limit = 32,
): Promise<{ profiles: ProfileRecord[]; unavailable: boolean }> {
  try {
    ...
    return {
      profiles: items
        ...
        .filter((item): item is ProfileRecord =>
          Boolean(item && profileId(item)),
        ),
      unavailable: false,
    };
  } catch {
...
export const resolvePublicProfile = cache(
  async (slug: string): Promise<PublicProfileResolution> => {
    const decodedSlug = decodePublicProfileSlug(slug).trim();
    if (!decodedSlug) return { status: 'not_found' };
    ...
    if (directId) {
      ...
      if (direct.status === 'found') {
        return {
          ...
          canonicalSlug: buildPublicProfileSlug(direct.profile),
        };
      }
    ...
    if (slugHandle) {
      ...
      if (
        direct.status === 'found' &&
        matchesPublicProfileSlug(decodedSlug, direct.profile)
      ) {
        return {
          status: 'found',
          profile: direct.profile,
    ...
    for (const variant of variants) {
      ...
      const candidate =
        result.profiles.find(item =>
          matchesPublicProfileSlug(decodedSlug, item),
        ) || null;
      if (candidate) {
        const detailed = await fetchProfileById(profileId(candidate));
        const profile =
          detailed.status === 'found' ? detailed.profile : candidate;
        return {
          status: 'found',
          profile,
          canonicalSlug: buildPublicProfileSlug(profile),
        };
      }
    }
...
export const getPublicProfileSocial = cache(
  async (userId: string): Promise<unknown | null> => {
