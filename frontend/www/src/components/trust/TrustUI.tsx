...
export function TrustBadge({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center rounded-full border border-[color:var(--app-accent-border)] text-[color:var(--app-accent)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] bg-[color:var(--app-accent-soft)] border-[color:var(--app-accent-border)] border-[color:var(--app-accent-border)] text-[color:var(--app-accent)]">
      {label}
    </span>
  );
}
...
export function LegalSummaryCard({
  ...
  return (
    <Link href={href} className="ui-panel-muted ui-card-hover block rounded-[22px] p-4">
      <h3 className="text-base font-bold text-[color:var(--app-text)]">{title}</h3>
      <p className="mt-2 text-sm leading-6 text-[color:var(--app-text-soft)]">{summary}</p>
      {bullets.length > 0 ? (
        <ul className="mt-3 space-y-1 text-[12px] text-[color:var(--app-text-soft)]">
          {bullets.slice(0, 2).map((item) => (
            <li key={item} className="flex items-start gap-2">
              <span className="mt-1 h-1.5 w-1.5 rounded-full text-[color:var(--app-accent)]" />
              <span>{item}</span>
            </li>
          ))}
        </ul>
        ...
        <ul className="mt-3 space-y-1 text-sm">
          {bullets.map((bullet) => (
            <li key={bullet} className="flex items-start gap-2">
              <span className="mt-2 h-1.5 w-1.5 rounded-full text-[color:var(--app-accent)]" />
              <span>{bullet}</span>
            </li>
          ))}
        </ul>
