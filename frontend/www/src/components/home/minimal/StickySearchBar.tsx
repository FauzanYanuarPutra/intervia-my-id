    ...
    <section className="ui-page-section ui-sticky-controls z-30">
      <div className="ui-feed-section rounded-[1.75rem] border border-[color:var(--app-border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(240,253,244,0.96))] p-2 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.4)] backdrop-blur dark:border-[color:var(--app-border)] dark:bg-[linear-gradient(180deg,rgba(24,24,24,0.96),rgba(10,10,10,0.94))]">
        <form onSubmit={onSearch} className="flex flex-col gap-2 sm:flex-row" role="search" aria-label="Sticky home search">
          ...
          <button
            type="submit"
            className="inline-flex min-h-[46px] items-center justify-center gap-2 rounded-[1.3rem] bg-[linear-gradient(135deg,var(--app-accent),var(--app-accent-strong))] px-5 text-sm font-black text-white shadow-[0_18px_36px_-26px_rgba(18,138,69,0.45)] transition hover:brightness-105"
          >
            {isId ? 'Cari' : 'Search'}
        ...
        <p className="px-2 pt-2 text-[11px] font-medium text-[color:var(--app-text-soft)] dark:text-[color:var(--app-text)]">
          {isId ? 'Cari dulu. Pilih. Lanjut chat.' : 'Start from a business need, then move into the most relevant suppliers, tools, services, or talent.'}
        </p>
      </div>
    </section>
  );
