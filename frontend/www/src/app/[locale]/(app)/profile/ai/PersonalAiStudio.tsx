  ...
  return (
    <div className="space-y-1.5">
      {lines.map((line, index) => {
        ...
        if (heading) {
          return (
            <p key={`h-${index}`} className="pt-1 text-[1.02em] font-bold leading-6">
        ...
        if (bullet) {
          return (
            <div key={`b-${index}`} className="grid grid-cols-[14px_minmax(0,1fr)] gap-1">
              <span className="pt-[0.42rem] text-[0.6em]">•</span>
        ...
        if (numbered) {
          return (
            <div key={`n-${index}`} className="grid grid-cols-[22px_minmax(0,1fr)] gap-1">
        ...
        return <p key={`p-${index}`}>{renderInlineMarkdown(line, `p-${index}`)}</p>;
      })}
    </div>
  );
}
  ...
  return (
    <div className="mb-2 grid gap-2">
      {media.map((item, index) => {
        ...
        const detail = [kind, formatBytes(item.size)].filter(Boolean).join(' / ');
          ...
          return (
            ...
            >
              ...
              <span className="flex min-w-0 items-center gap-2 px-2 py-1.5 text-[11px] font-semibold">
                ...
                {detail ? <span className="shrink-0 opacity-70">{detail}</span> : null}
              </span>
            </a>
          );
        }
        if (kind === 'video' && item.url) {
          return (
            <div key={key} className="overflow-hidden rounded-[14px] bg-black/6 dark:bg-white/10">
              <video
                src={item.url}
                className="max-h-[260px] w-full min-w-[180px] object-cover"
                controls
...
function makeDraft(agent: PersonalAgent | null): SettingsDraft {
  ...
  return {
    ...
    builder_config: agent.builder_config || createDefaultPersonalAiBuilderConfig(),
...
export default function PersonalAiStudio() {
  ...
  async function uploadPersonalAiMedia(file: File): Promise<StoredPersonalAiMedia | null> {
    ...
    if (!res.ok) {
      throw new Error(payload.details || payload.error || 'Upload media gagal.');
    }
  ...
  async function handleAttachmentChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const availableSlots = Math.max(0, MAX_AI_ATTACHMENTS - draftAttachments.length);
    if (availableSlots <= 0) {
      setError(isId ? 'Maksimal 4 media per pesan.' : 'Maximum 4 media per message.');
      return;
    }
    ...
    if (next.length > 0) {
      setDraftAttachments(current => [...current, ...next].slice(0, MAX_AI_ATTACHMENTS));
    }
  }
  ...
  async function sendMessage(
    text = input,
    options: { actionInstruction?: string; clearInput?: boolean } = {},
  ) {
    ...
    const message =
      ...
      (activeAttachments.length > 0
        ...
        : '');
    if ((!message && activeAttachments.length === 0) || !selectedAgent || sending) return;
    setSending(true);
    setError('');
  ...
  function applyBuilderTemplate(templateId: string) {
    const template = PERSONAL_AI_BUILDER_TEMPLATES.find(item => item.id === templateId);
  ...
  function updateBuilderConfig(
    ...
    setDraft(current => ({
      ...current,
      builder_config: updater(current.builder_config || createDefaultPersonalAiBuilderConfig()),
    }));
  }
  ...
  if (authLoading || loadingAgents) {
    return (
      <main className="flex h-[var(--app-viewport-height)] min-h-0 items-center justify-center overflow-hidden bg-[color:var(--app-surface-muted)] px-4">
  ...
  const panels: PersonalAiPanel[] = ['chat', 'builder', 'settings', 'share', 'memory'];
  ...
  return (
    <main className="h-[var(--app-viewport-height)] max-h-[var(--app-viewport-height)] min-h-0 overflow-hidden overscroll-none bg-[#d9dbd5] text-[color:var(--app-text)] dark:bg-[#0b141a]">
          ...
          >
                  ...
                  {messages.length === 0 ? (
                    ...
                    <div className="mx-auto grid max-w-3xl gap-3">
                      {messages.map(message => (
                        ...
                        >
                          <MessageMediaList media={mediaFromMessage(message)} />
                ...
                <div className="shrink-0 border-t border-black/5 bg-[#f0f2f5] px-2 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-2 dark:border-white/6 dark:bg-[#202c33] sm:px-3 lg:pb-3">
                  ...
                  <div className="flex min-w-0 items-end gap-2">
                    <button
                      ...
                      disabled={sending || draftAttachments.length >= MAX_AI_ATTACHMENTS}
                    ...
                    <button
                      ...
                      disabled={sending || (!input.trim() && draftAttachments.length === 0)}
            ...
            {activePanel !== 'chat' ? (
              <div className="min-h-0 flex-1 overflow-y-auto p-4">
                {activePanel === 'builder' ? (() => {
                  const config = draft.builder_config || createDefaultPersonalAiBuilderConfig();
                  const selectedStep =
                    config.steps.find(step => step.id === selectedBuilderStepId) ||
                    config.steps[0];
                  return (
                    <div className="mx-auto grid max-w-5xl gap-4">
                      {!canEditSelected ? (
                        <div className="rounded-[14px] border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800">
                          {isId ? 'Builder hanya bisa diedit pemilik AI.' : 'Only the AI owner can edit the builder.'}
                        </div>
                      ) : null}
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="text-sm font-bold">
                              {isId ? 'AI Studio Builder' : 'AI Studio Builder'}
                            </p>
                        ...
                        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                          {PERSONAL_AI_BUILDER_TEMPLATES.map(template => (
                            <button
                              ...
                              onClick={() => applyBuilderTemplate(template.id)}
                              className={cn(
                      ...
                      <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)_280px]">
                          ...
                          <div className="grid gap-2">
                            {config.steps.map((step, index) => (
                              <button
                                key={step.id}
                                type="button"
                                onClick={() => setSelectedBuilderStepId(step.id)}
                                className={cn(
                            ...
                            <div className="mt-4 rounded-[16px] border border-[color:var(--app-border)] bg-[color:var(--app-surface-strong)] p-3">
                              ...
                              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                                <p className="text-xs font-bold text-[color:var(--app-text-soft)]">
                                  {isId ? 'Field di step ini' : 'Fields in this step'}
                                </p>
                                <button
                                  type="button"
                                  disabled={!canEditSelected}
                                  ...
                                  >
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                      <input
                                        ...
                                        disabled={!canEditSelected}
                                        onChange={event =>
                                          updateSelectedBuilderBlock(block.id, current => ({
                                            ...current,
                                            label: event.target.value,
                                            variable:
                                              current.variable ||
                                              variableKeyFromLabel(event.target.value),
                                          }))
                                        }
                                      ...
                                      <div className="flex shrink-0 items-center gap-1.5">
                                        <select
                                          ...
                                          onChange={event =>
                                            updateSelectedBuilderBlock(block.id, current => {
                                              const nextType = event.target.value as AIBuilderBlockType;
                                              return {
                                                ...current,
                                                type: nextType,
                                                options:
                                        ...
                                        <button
                                          type="button"
                                          disabled={!canEditSelected || selectedStep.blocks.length <= 1}
                                          ...
                                          aria-label={isId ? 'Hapus field' : 'Remove field'}
                                        >
                                    ...
                                    <div className="mt-2 grid gap-2 sm:grid-cols-2">
                                      <label className="grid gap-1 text-[10px] font-bold text-[color:var(--app-text-soft)]">
                                        ...
                                        <input
                                          ...
                                          onChange={event =>
                                            updateSelectedBuilderBlock(block.id, current => ({
                                              ...current,
                                              variable: variableKeyFromLabel(event.target.value),
                                            }))
                                          }
                                      ...
                                      <label className="grid gap-1 text-[10px] font-bold text-[color:var(--app-text-soft)]">
                                        Placeholder / help
                                        <input
                                          value={block.placeholder || block.helpText || ''}
                                          disabled={!canEditSelected}
                                          onChange={event =>
                                            updateSelectedBuilderBlock(block.id, current => ({
                                              ...current,
                                    ...
                                    {block.options?.length ? (
                                      <div className="mt-2 flex flex-wrap gap-1.5">
                                        {block.options.slice(0, 8).map(option => (
                                          <span
                                            key={option.id}
                        ...
                        <section className="rounded-[18px] border border-[color:var(--app-border)] bg-[color:var(--app-surface)] p-3">
                          ...
                          <div className="mt-4 rounded-[14px] border border-[color:var(--app-border)] bg-[color:var(--app-surface-strong)] p-3">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <button
                                ...
                                aria-label={isId ? 'Tambah output' : 'Add output'}
                              >
                                <Plus className="h-3.5 w-3.5" />
                              </button>
                            </div>
                            <div className="mt-2 grid gap-1.5">
                              {config.output.sections.slice(0, 12).map(section => (
                                <div
                                  key={section.id}
                                ...
                                >
                                  <div className="flex items-center gap-1.5">
                                    <input
                                      ...
                                      disabled={!canEditSelected}
                                      onChange={event =>
                                        updateOutputSection(section.id, current => ({
                                          ...current,
                                          title: event.target.value,
                                          key: current.key || variableKeyFromLabel(event.target.value),
                                        }))
                                      }
                                    ...
                                    <button
                                      type="button"
                                      disabled={!canEditSelected || config.output.sections.length <= 1}
                                      onClick={() => removeOutputSection(section.id)}
                                      ...
                                      aria-label={isId ? 'Hapus output' : 'Remove output'}
                                    >
                                  ...
                                  <div className="grid gap-1 sm:grid-cols-2">
                                    <select
                                      ...
                                      onChange={event =>
                                        updateOutputSection(section.id, current => ({
                                          ...current,
                                          type: event.target.value as AIOutputSection['type'],
                                        }))
                                      }
                                    ...
                                    <input
                                      ...
                                      onChange={event =>
                                        updateOutputSection(section.id, current => ({
                                          ...current,
                                          key: variableKeyFromLabel(event.target.value),
                                        }))
                                      }
                                      className="rounded-[10px] border border-[color:var(--app-border)] bg-[color:var(--app-surface)] px-2 py-1.5 text-[10px] font-bold text-[color:var(--app-text-soft)]"
                          ...
                          <div className="mt-2 grid gap-2">
                            {PERSONAL_AI_MODEL_REGISTRY.slice(0, 6).map(model => (
                              <div
                                key={model.id}
                  ...
                  <div className="mx-auto grid max-w-3xl gap-3">
                    ...
                    <p className="text-[11px] leading-5 text-[color:var(--app-text-soft)]">
                      {isId
                        ...
                        : 'Private is owner-only. Unlisted opens by link. Public is ready for Lajukan profile/search discovery.'}
                    </p>
                    {canEditSelected ? (
                  ...
                  <div className="mx-auto grid max-w-2xl gap-3">
                    <div className="rounded-[16px] border border-[color:var(--app-border)] bg-[color:var(--app-surface)] p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-[color:var(--app-text-soft)]">
                        {normalizeUiVisibility(selectedAgent?.visibility || 'private') === 'private'
                          ? isId
                            ? 'AI ini masih private dan tidak bisa dibuka orang lain.'
                            : 'This AI is private and cannot be opened by others.'
                          : normalizeUiVisibility(selectedAgent?.visibility || 'private') === 'public'
                            ? isId
