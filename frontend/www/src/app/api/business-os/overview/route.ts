...
import { BUSINESS_MODULES, type BusinessModuleStatus } from '@/lib/business/moduleCatalog';
...
export async function GET(req: NextRequest) {
  ...
  const [contentResult, transactionResult, leadResult, supportResult, inboxResult] =
    await Promise.all([
      fetchListFromUpstream(`${MARKETPLACE_URL}/v1/content?limit=200&offset=0`, token),
      fetchListFromUpstream(`${MARKETPLACE_URL}/v1/transactions?limit=200&offset=0`, token),
      fetchListFromUpstream(`${MARKETPLACE_URL}/v1/crm/leads?limit=200&offset=0`, token),
      fetchListFromUpstream(`${MARKETPLACE_URL}/v1/support/tickets?limit=200&offset=0`, token),
    ...
    flow_recommendations: [
      {
        ...
        description: 'Capture lead in CRM, close via chat, and execute via transactions.',
        steps: [
      ...
      },
      {
        ...
        description: 'Resolve support issues fast and feed insights into knowledge base.',
        steps: [
      ...
      },
      {
        ...
        description: 'Strengthen listings and measure business momentum in analytics.',
        steps: [
