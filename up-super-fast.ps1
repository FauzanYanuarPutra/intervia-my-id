...
# Keep existing profile env values, but edge/tunnel are explicit opt-ins because
# external connectivity must never destabilize normal local startup.
$profiles = @()
...
# Remove external profiles from inherited/general profile settings. Dedicated
# opt-in below can put them back without deleting any env setting from disk.
$profiles = @($profiles | Where-Object { $_ -notin @("tunnel", "edge") })

if ($aiEnabled -and $profiles -notcontains "ai") {
    $profiles += "ai"
}

$devTunnelSetting = $env:LAJUKAN_DEV_TUNNEL
if ([string]::IsNullOrWhiteSpace($devTunnelSetting)) {
    $devTunnelSetting = Get-EnvFileValue -Path $EnvFile -Name "LAJUKAN_DEV_TUNNEL"
}
$devEdgeSetting = $env:LAJUKAN_DEV_EDGE
if ([string]::IsNullOrWhiteSpace($devEdgeSetting)) {
    $devEdgeSetting = Get-EnvFileValue -Path $EnvFile -Name "LAJUKAN_DEV_EDGE"
}

$enableTunnel = $WithTunnel -or (Test-TruthyEnvValue -Value $devTunnelSetting)
$enableEdge = $WithEdge -or (Test-TruthyEnvValue -Value $devEdgeSetting)

if ($NoTunnel) { $enableTunnel = $false }
if ($NoEdge) { $enableEdge = $false }

if ($enableTunnel -and $profiles -notcontains "tunnel") {
    $profiles += "tunnel"
}
if ($enableEdge -and $profiles -notcontains "edge") {
    $profiles += "edge"
}
...
try {
    ...
    Write-Host "Automatic recovery checkpoint warning: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Host "Startup continues without destructive reset; existing named volumes remain untouched." -ForegroundColor Yellow
}
