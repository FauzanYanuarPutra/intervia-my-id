...
| Search | Yes |  |  | www/marketplace/community/Meili | `/api/content`, `/v1/content`, forum/community search | `content_items`, GIN/trigram, Meili | `/search` | Taxonomy/index drift | Define canonical search intent schema |
...
| Mencari/Menawarkan |  | Yes |  | www/marketplace | create/content/offers/requests APIs | content metadata, offers/request-related migrations | `/create/[flow]` | Flow taxonomy can drift | Align create templates with DB metadata |
| Kategori |  | Yes |  | www/marketplace | content/search/create APIs | metadata indexes | home/search/create | Mixed labels/promotional badges risk | Maintain taxonomy doc |
| Lokasi |  | Yes |  | www/marketplace/super-app | locations, stores, search APIs | lat/lng metadata indexes, tracking tables | search/umkm/map | Text location vs coordinates can be confused | Only show distance with coordinates |
...
| Reels | Yes |  |  | community/www | `/api/reels`, `/v1/reels` | reels/comments/actions/events | `/reels` | Listing/profile linkage may be partial | Require explicit metadata links |
