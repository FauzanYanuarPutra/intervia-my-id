...
$ScriptVersion = "2026.08.21-one-command-safe-3.1.0"
...
# Fail early on unsupported PowerShell versions and verify this file can be parsed
# by the exact PowerShell runtime executing it.
...
try {
    ...
    if ($parseErrors -and $parseErrors.Count -gt 0) {
        ...
        throw "PowerShell self-parse failed at line $($firstParseError.Extent.StartLineNumber): $($firstParseError.Message)"
    }
}
catch {
    if ($_.Exception.Message -like "PowerShell self-parse failed*") {
        throw
    }
    Write-Host "PowerShell parser self-check skipped: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
# Stable daily default: core + local AI only. External edge/tunnel services are
# opt-in so internet/Cloudflare state cannot break ordinary local startup.
$profiles = @()
...
# Tunnel/edge are deliberately explicit. A stale token or stale COMPOSE_PROFILES
# must not silently turn an external dependency into part of daily startup.
if ($WithTunnel -and -not $NoTunnel) {
    $profiles += "tunnel"
}
if ($WithEdge -and -not $NoEdge) {
    $profiles += "edge"
}
if ($WithTunnel -and -not $NoEdge) {
    # A tunnel usually fronts the local edge proxy.
    $profiles += "edge"
}
...
$profiles = @(
    $profiles |
    Where-Object {
        (-not $NoTunnel -or $_ -ne "tunnel") -and
        (-not $NoEdge -or $_ -ne "edge")
    } |
    Select-Object -Unique
)

$env:COMPOSE_PROFILES = ($profiles -join ",")
Write-Host "Compose profiles: $($env:COMPOSE_PROFILES)" -ForegroundColor DarkGray
...
function Invoke-AutomaticBackupCore {
    param(
        [Parameter(Mandatory = $true)]
        [string]$ProjectName,
        [Parameter(Mandatory = $true)]
    ...
    foreach ($target in $postgresTargets) {
        ...
        if (-not $ok) {
            $failedNames += [string]$target.service
        }
    }
    ...
    $manifest = [ordered]@{
        ...
        failures = @($failedNames | ForEach-Object { [string]$_ })
...
function Invoke-AutomaticBackup {
    ...
    try {
        Invoke-AutomaticBackupCore `
            -ProjectName $ProjectName `
            -EnvFilePath $EnvFilePath
    }
    catch {
        # Backup is a safety layer; an implementation/runtime quirk must never
        # destroy or reset data, nor make a healthy local stack impossible to start.
        # Named Docker volumes remain authoritative and this script never deletes them.
        Write-Host "Automatic backup warning: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host "Startup continues without destructive reset; existing named volumes are preserved." -ForegroundColor Yellow
        $script:LastBackupPath = ""
    }
}
...
function Wait-ComposeServicesReady {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        foreach ($serviceName in $Services) {
            ...
            if ($containerIds.Count -eq 0) {
                $pending += "${serviceName}: container not created"
                continue
            }
            ...
            foreach ($containerId in $containerIds) {
                ...
                if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($stateLine)) {
                    $pending += "${serviceName}: inspect unavailable"
                    continue
                }
                ...
                if ($oneShotServices.ContainsKey($serviceName)) {
                    ...
                    if ($containerState -in @("exited", "dead")) {
                        $terminalFailures += "${serviceName}: state=$containerState exit=$exitCode"
                    }
                    else {
                        $pending += "${serviceName}: state=$containerState"
                    }
                    continue
                }
                ...
                elseif ($containerState -eq "restarting" -and $restartCount -ge 3) {
                    $terminalFailures += "${serviceName}: restart loop count=$restartCount"
                }
                else {
                    $pending += "${serviceName}: state=$containerState health=$healthState"
                }
            }
        }
