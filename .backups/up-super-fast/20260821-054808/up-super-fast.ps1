...
$ScriptVersion = "2026.08.21-one-command-safe-3.0.1"
...
Write-Host "PowerShell runtime: $($PSVersionTable.PSVersion) | native-array compatibility mode." -ForegroundColor DarkGray
...
function Start-ComposeServicesFast {
    ...
    foreach ($serviceName in $Services) {
        ...
        if ($serviceContainerIds.Count -eq 0) {
            $missingServices += [string]$serviceName
            continue
        }
...
try {
    ...
    Write-Host "Startup will continue because this script never deletes named volumes." -ForegroundColor Yellow
}
