...
try {
    ...
    function Test-DockerEngineHealthy {
        ...
        # A healthy probe is exactly the JSON-quoted server version, e.g. "29.6.2".
        # Docker Desktop can occasionally return HTTP 500 while the wrapper still
        # yields a zero process code, so inspect the payload instead of trusting
        # only $LASTEXITCODE.
        if ($Text -match '(?i)(500 Internal Server Error|API route and version|_ping|dockerDesktopLinuxEngine|error response from daemon|Cannot connect to the Docker daemon)') {
            return $false
        }
        ...
        if ($BuildTargets.Count -eq 0) {
            ...
            try {
                ...
                elseif ($BuildTargets.Count -gt 12) {
                    ...
                    # Serialize the solver for reliability; the previous 2-worker
                    # mode still allowed Docker Desktop's BuildKit frontend to die.
                    $AdaptiveLimit = 1
                }
                elseif ($HostCpu -le 4) {
                    $AdaptiveLimit = [math]::Min($ParallelLimit, 2)
                ...
                if ($BuildProbe.ExitCode -eq 0) {
                    ...
                    $PostBuildProbe = Invoke-DockerNative -Arguments @("info", "--format", "{{json .ServerVersion}}") -Silent
                    if (-not (Test-DockerEngineHealthy -ProbeOutput $PostBuildProbe.Output -ExitCode $PostBuildProbe.ExitCode)) {
                        $EngineFailure = $true
                    }
                    ...
                    if ($EngineFailure -or $BuildKitFailure) {
                        ...
                        foreach ($ServiceName in $BuildTargets) {
                            ...
                            for ($ServiceAttempt = 1; $ServiceAttempt -le 2; $ServiceAttempt++) {
                                ...
                                $RetryProbe = Invoke-DockerNative -Arguments @("info", "--format", "{{json .ServerVersion}}") -Silent
                                if (-not (Test-DockerEngineHealthy -ProbeOutput $RetryProbe.Output -ExitCode $RetryProbe.ExitCode)) {
    ...
    function Invoke-DockerEngineRecovery {
        ...
        $DockerRecoveryState.AttemptCount++
        $DockerRecoveryState.Reason = $Reason
        Write-Warning "Docker Engine/BuildKit gagal pada saat $Reason. Mencoba bounded recovery Docker Desktop (attempt $($DockerRecoveryState.AttemptCount)/4)..."

        # Restart is allowed to report a timeout while Desktop is still transitioning.
        # Do not immediately call that state healthy; wait for the actual Engine API.
        $RestartProbe = Invoke-DockerNative -Arguments @("desktop", "restart", "--timeout", "180")
        $RestartDetails = ($RestartProbe.Output -join " ").Trim()
        if ($RestartProbe.ExitCode -ne 0) {
            Write-Warning "docker desktop restart belum selesai: $RestartDetails"
        }

        for ($RecoveryAttempt = 1; $RecoveryAttempt -le 36; $RecoveryAttempt++) {
            Start-Sleep -Seconds 5
            $RecoveryProbe = Invoke-DockerNative -Arguments @("info", "--format", "{{json .ServerVersion}}") -Silent
            if (Test-DockerEngineHealthy -ProbeOutput $RecoveryProbe.Output -ExitCode $RecoveryProbe.ExitCode) {
                $DockerRecoveryState.Succeeded = $true
                $DockerRecoveryState.Reason = "recovered"
                $EngineVersion = (($RecoveryProbe.Output -join " ").Trim())
                Write-Host "Docker Engine kembali sehat setelah recovery ($EngineVersion)." -ForegroundColor Green
                return $true
            }
        }

        Write-Warning "Docker Engine belum pulih setelah restart. Mencoba docker desktop start..."
        $StartProbe = Invoke-DockerNative -Arguments @("desktop", "start", "--timeout", "180")
        $StartDetails = ($StartProbe.Output -join " ").Trim()
        if ($StartProbe.ExitCode -ne 0) {
            Write-Warning "docker desktop start gagal: $StartDetails"
        }
        ...
        for ($RecoveryAttempt = 1; $RecoveryAttempt -le 36; $RecoveryAttempt++) {
            ...
            if (Test-DockerEngineHealthy -ProbeOutput $RecoveryProbe.Output -ExitCode $RecoveryProbe.ExitCode) {
                $DockerRecoveryState.Succeeded = $true
                $DockerRecoveryState.Reason = "recovered-after-start"
                $EngineVersion = (($RecoveryProbe.Output -join " ").Trim())
                Write-Host "Docker Engine kembali sehat setelah start fallback ($EngineVersion)." -ForegroundColor Green
                return $true
            }
        }
