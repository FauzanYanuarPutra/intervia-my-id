...
CREATE INDEX IF NOT EXISTS idx_user_profiles_search_surface_tsv
...
ON user_profiles USING GIN ((metadata->'roles'))
WHERE metadata ? 'roles';

CREATE INDEX IF NOT EXISTS idx_user_profiles_provider_skills_gin
ON user_profiles USING GIN ((metadata->'provider_profile'->'skills'))
