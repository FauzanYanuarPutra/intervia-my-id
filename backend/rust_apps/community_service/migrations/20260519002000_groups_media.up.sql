CREATE TABLE IF NOT EXISTS lajukan_groups (
  ...
  posting_permission text NOT NULL DEFAULT 'member' CHECK (posting_permission IN ('public', 'member', 'moderator')),
  membership_permission text NOT NULL DEFAULT 'open' CHECK (membership_permission IN ('open', 'approval', 'invite')),
  cover_url text NULL,
...
CREATE INDEX IF NOT EXISTS lajukan_groups_discovery_idx
  ON lajukan_groups (status, privacy, updated_at DESC);

CREATE INDEX IF NOT EXISTS lajukan_groups_search_idx
  ON lajukan_groups
  USING gin (to_tsvector('simple', coalesce(name, '') || ' ' || coalesce(description, '') || ' ' || coalesce(slug, '')));
...
CREATE INDEX IF NOT EXISTS lajukan_group_members_user_idx
  ON lajukan_group_members (user_id, status, updated_at DESC);
...
SELECT g.id, u.id,
  CASE WHEN u.id = 'u-1' THEN 'owner' ELSE 'member' END,
  'active',
  now(),
  now()
FROM lajukan_groups g
CROSS JOIN lajukan_forum_users u
WHERE g.slug IN ('announcements', 'product-ux', 'marketplace-projects', 'community-events', 'support-help')
