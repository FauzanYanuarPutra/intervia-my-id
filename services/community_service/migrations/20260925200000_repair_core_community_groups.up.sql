...
-- Idempotent repair for development databases that were created before the
-- official core community-group seed was applied. No demo users/content are
-- created; only platform-owned discovery groups are restored.

INSERT INTO forum.lajukan_forum_users (
  id, username, name, avatar_url, title, reputation, base_reputation,
  badges, metadata, created_at, updated_at
)
VALUES (
