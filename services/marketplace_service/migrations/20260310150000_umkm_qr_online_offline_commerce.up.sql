...
CREATE TABLE IF NOT EXISTS umkm_stores (
  ...
  lat DOUBLE PRECISION NOT NULL CHECK (
    lat BETWEEN -90 AND 90
  ),
  lng DOUBLE PRECISION NOT NULL CHECK (
...
CREATE INDEX IF NOT EXISTS idx_umkm_stores_city_active ON umkm_stores (city, is_active, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_umkm_stores_owner ON umkm_stores (owner_user_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_umkm_stores_lat_lng ON umkm_stores (lat, lng);
CREATE INDEX IF NOT EXISTS idx_umkm_stores_metadata_gin ON umkm_stores USING GIN (metadata);
...
CREATE INDEX IF NOT EXISTS idx_umkm_products_store_available ON umkm_products (
  store_id,
  is_available,
  category,
  updated_at DESC
);
CREATE INDEX IF NOT EXISTS idx_umkm_products_metadata_gin ON umkm_products USING GIN (metadata);
CREATE TABLE IF NOT EXISTS umkm_tables (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  store_id UUID NOT NULL REFERENCES umkm_stores(id) ON DELETE CASCADE,
  table_code TEXT NOT NULL,
  capacity INT NOT NULL DEFAULT 2 CHECK (
    capacity > 0
    AND capacity <= 40
  ),
...
CREATE INDEX IF NOT EXISTS idx_umkm_tables_store_status ON umkm_tables (store_id, status, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_umkm_qr_tokens_store_mode ON umkm_qr_tokens (store_id, mode, is_active, updated_at DESC);
CREATE TABLE IF NOT EXISTS umkm_orders (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  store_id UUID NOT NULL REFERENCES umkm_stores(id) ON DELETE CASCADE,
  channel TEXT NOT NULL CHECK (channel IN ('online', 'offline')),
  table_id UUID NULL REFERENCES umkm_tables(id) ON DELETE
  SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_umkm_orders_store_time ON umkm_orders (store_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_umkm_orders_store_status ON umkm_orders (
  store_id,
  status,
  payment_status,
  updated_at DESC
);
CREATE INDEX IF NOT EXISTS idx_umkm_orders_table_open ON umkm_orders (
  table_id,
  payment_status,
  status,
  created_at DESC
...
CREATE INDEX IF NOT EXISTS idx_umkm_orders_metadata_gin ON umkm_orders USING GIN (metadata);
CREATE TABLE IF NOT EXISTS umkm_order_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id UUID NOT NULL REFERENCES umkm_orders(id) ON DELETE CASCADE,
  product_id UUID NULL REFERENCES umkm_products(id) ON DELETE
  SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_umkm_order_items_order ON umkm_order_items (order_id, created_at ASC);
CREATE TABLE IF NOT EXISTS umkm_table_sessions (
  ...
  status TEXT NOT NULL DEFAULT 'open' CHECK (
    status IN ('open', 'closed', 'moved', 'cancelled')
  ),
  metadata JSONB NOT NULL DEFAULT '{}'::jsonb
);
CREATE INDEX IF NOT EXISTS idx_umkm_table_sessions_store ON umkm_table_sessions (store_id, opened_at DESC);
CREATE INDEX IF NOT EXISTS idx_umkm_table_sessions_table_open ON umkm_table_sessions (table_id, status, opened_at DESC);
-- -------------------------------------------------------------------
