...
fn branch_code_rejects_blank_or_unsafe_values() {
    ...
    assert!(matches!(
        normalize_branch_code("a/b"),
        Err(GovernanceError::Validation(_))
    ));
    assert!(matches!(
        normalize_branch_code("branch code"),
        Err(GovernanceError::Validation(_))
    ));
}
...
fn branch_kind_is_closed_to_canonical_values() {
    ...
    assert!(matches!(
        validate_branch_kind("legal_entity"),
        Err(GovernanceError::Validation(_))
    ));
}
...
async fn creator_gets_technical_access_without_legal_facts_being_inferred(pool: PgPool) {
    ...
    let inferred_relationships: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM business_relationships WHERE business_id = $1")
            .bind(business_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    let inferred_jurisdictions: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM business_jurisdictions WHERE business_id = $1")
            .bind(business_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    let inferred_legal_profiles: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM business_legal_profiles WHERE business_id = $1")
            .bind(business_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    assert_eq!(
        ...
        "technical creator must not become a legal owner relationship automatically"
    );
    assert_eq!(
        inferred_jurisdictions, 0,
        "jurisdiction must require recorded evidence"
    );
    assert_eq!(
        inferred_legal_profiles, 0,
        "display name must not become a legal profile automatically"
    );
}
