...
impl Wave2Repository {
    ...
    pub(crate) async fn get_finance_plan(&self, business_id: Uuid, organization_id: Uuid) -> Result<Option<FinancePlanRecord>, Wave2RepositoryError> {
    ...
    pub(crate) async fn upsert_finance_plan(&self, business_id: Uuid, organization_id: Uuid, request: FinancePlanRequest) -> Result<FinancePlanRecord, Wave2RepositoryError> {
    ...
    pub(crate) async fn list_obligations(&self, business_id: Uuid, organization_id: Uuid) -> Result<Vec<ObligationRecord>, Wave2RepositoryError> {
    ...
    pub(crate) async fn create_obligation(&self, actor_id: Uuid, business_id: Uuid, organization_id: Uuid, request: CreateObligationRequest) -> Result<ObligationRecord, Wave2RepositoryError> {
        ...
        if request.amount <= 0 || request.interval_days <= 0 || request.interval_days > 3660 { return Err(Wave2RepositoryError::Validation("invalid_obligation")); }
    ...
    pub(crate) async fn pay_obligation(&self, actor_id: Uuid, business_id: Uuid, organization_id: Uuid, obligation_id: Uuid, idempotency_key: Uuid, paid_on: NaiveDate) -> Result<ObligationPaymentOutcome, Wave2RepositoryError> {
        if let Some(existing) = load_payment(&self.db, business_id, idempotency_key).await? { return Ok(ObligationPaymentOutcome { payment: existing, replayed: true }); }
        ...
        let due_after = obligation.next_due_on.checked_add_days(chrono::Days::new(obligation.interval_days as u64)).ok_or(Wave2RepositoryError::Validation("obligation_due_overflow"))?;
        ...
        if inserted.is_none() { tx.rollback().await?; let existing=load_payment(&self.db,business_id,idempotency_key).await?.ok_or(Wave2RepositoryError::Conflict)?; return Ok(ObligationPaymentOutcome{payment:existing,replayed:true}); }
        ...
        sqlx::query("UPDATE business_obligation_payments SET finance_entry_id=$2 WHERE id=$1").bind(payment_id).bind(finance_id).execute(&mut *tx).await?;
        ...
        let payment = load_payment_tx(&mut tx, payment_id).await?.ok_or(Wave2RepositoryError::Database)?;
        tx.commit().await?;
        Ok(ObligationPaymentOutcome { payment, replayed: false })
    }

    pub(crate) async fn create_purchase(&self, actor_id: Uuid, business_id: Uuid, organization_id: Uuid, idempotency_key: Uuid, request: CreatePurchaseRequest) -> Result<PurchaseOutcome, Wave2RepositoryError> {
        if request.stock_quantity_delta <= Decimal::ZERO || request.total_amount <= 0 { return Err(Wave2RepositoryError::Validation("invalid_purchase")); }
        let account_key=normalized_account(&request.account_key)?;
        let note = normalized_optional_text(&request.note, 2000, "purchase_note_too_long")?;
        if let Some(existing)=load_purchase(&self.db,business_id,idempotency_key).await? { return Ok(PurchaseOutcome{purchase:existing,replayed:true}); }
        let mut tx=self.db.begin().await?;
        ...
        if inserted.is_none() { tx.rollback().await?; if let Some(existing)=load_purchase(&self.db,business_id,idempotency_key).await? { return Ok(PurchaseOutcome{purchase:existing,replayed:true}); } return Err(Wave2RepositoryError::NotFound); }
        ...
        sqlx::query(r#"INSERT INTO business_inventory_movements (business_id,organization_id,ingredient_id,movement_type,quantity_delta,quantity_before,quantity_after,source_type,source_id,note,created_by_user_id) VALUES ($1,$2,$3,'purchase',$4,$5,$6,'business_purchase',$7,$8,$9)"#)
        ...
        sqlx::query("UPDATE business_purchases SET finance_entry_id=$2 WHERE id=$1").bind(purchase_id).bind(finance_id).execute(&mut *tx).await?;
        let purchase=load_purchase_tx(&mut tx,purchase_id).await?.ok_or(Wave2RepositoryError::Database)?;
        tx.commit().await?;
        Ok(PurchaseOutcome{purchase,replayed:false})
    }

    pub(crate) async fn open_cash_shift(&self, actor_id:Uuid,business_id:Uuid,organization_id:Uuid,request:OpenCashShiftRequest)->Result<CashShiftRecord,Wave2RepositoryError>{
        if request.opening_cash<0{return Err(Wave2RepositoryError::Validation("invalid_opening_cash"));}
    ...
    pub(crate) async fn close_cash_shift(&self, actor_id:Uuid,business_id:Uuid,organization_id:Uuid,shift_id:Uuid,request:CloseCashShiftRequest)->Result<CashShiftRecord,Wave2RepositoryError>{
        if request.actual_cash<0{return Err(Wave2RepositoryError::Validation("invalid_actual_cash"));}
        let note=normalized_optional_text(&request.note,2000,"cash_shift_note_too_long")?;
        let mut tx=self.db.begin().await?;
        ...
        if shift.closed_at.is_some(){return Err(Wave2RepositoryError::Conflict);}
        ...
        let expected=shift.opening_cash.checked_add(movement).ok_or(Wave2RepositoryError::Validation("cash_shift_amount_overflow"))?;
        let variance=request.actual_cash.checked_sub(expected).ok_or(Wave2RepositoryError::Validation("cash_shift_amount_overflow"))?;
        ...
        tx.commit().await?; Ok(closed)
    }

    pub(crate) async fn set_primary_material(&self,actor_id:Uuid,business_id:Uuid,organization_id:Uuid,product_id:Uuid,request:SetPrimaryMaterialRequest)->Result<PrimaryMaterialRecord,Wave2RepositoryError>{
        if request.expected_input_quantity<=Decimal::ZERO||request.expected_output_units<=Decimal::ZERO{return Err(Wave2RepositoryError::Validation("invalid_primary_material_yield"));}
    ...
    pub(crate) async fn create_yield_observation(&self,actor_id:Uuid,business_id:Uuid,organization_id:Uuid,request:CreateYieldObservationRequest)->Result<YieldObservationRecord,Wave2RepositoryError>{
        if request.input_quantity<=Decimal::ZERO||request.output_units<=Decimal::ZERO{return Err(Wave2RepositoryError::Validation("invalid_yield_observation"));}
...
fn validate_finance_plan(request:&FinancePlanRequest)->Result<(),Wave2RepositoryError>{
    let values=[request.owner_payroll_bps,request.staff_payroll_bps,request.working_capital_bps,request.operations_bps,request.reserve_bps];
    if values.iter().any(|value| !(0..=10000).contains(value)) { return Err(Wave2RepositoryError::Validation("invalid_finance_allocation")); }
    let total:i32=values.iter().sum(); if total>10000{return Err(Wave2RepositoryError::Validation("finance_allocation_exceeds_100_percent"));} Ok(())
}

fn canonical_expense_type(value:&str)->Result<&'static str,Wave2RepositoryError>{
    let canonical=canonical_manual_finance_entry_type(value).map_err(|_|Wave2RepositoryError::Validation("invalid_obligation_entry_type"))?;
    if canonical.ends_with("_expense") { Ok(canonical) } else { Err(Wave2RepositoryError::Validation("obligation_must_be_expense")) }
}
fn normalized_account(value:&str)->Result<String,Wave2RepositoryError>{let value=value.trim().to_ascii_lowercase();if matches!(value.as_str(),"cash"|"bank"|"ewallet"|"payable"){Ok(value)}else{Err(Wave2RepositoryError::Validation("invalid_finance_account"))}}
fn normalized_text(value:&str,max:usize,code:&'static str)->Result<String,Wave2RepositoryError>{let value=value.split_whitespace().collect::<Vec<_>>().join(" ");if value.is_empty()||value.chars().count()>max{Err(Wave2RepositoryError::Validation(code))}else{Ok(value)}}
fn normalized_optional_text(value:&str,max:usize,code:&'static str)->Result<String,Wave2RepositoryError>{if value.chars().count()>max{Err(Wave2RepositoryError::Validation(code))}else{Ok(value.trim().to_owned())}}
fn default_cash()->String{"cash".to_owned()}
fn is_unique_violation(error:&sqlx::Error)->bool{matches!(error,sqlx::Error::Database(db) if db.is_unique_violation())}

async fn load_payment(pool:&PgPool,business_id:Uuid,key:Uuid)->Result<Option<ObligationPaymentRecord>,Wave2RepositoryError>{sqlx::query_as::<_,ObligationPaymentRecord>("SELECT id,business_id,organization_id,obligation_id,idempotency_key,finance_entry_id,paid_amount,paid_on,due_on_before,due_on_after,created_by_user_id,created_at FROM business_obligation_payments WHERE business_id=$1 AND idempotency_key=$2").bind(business_id).bind(key).fetch_optional(pool).await.map_err(Into::into)}
async fn load_payment_tx(tx:&mut Transaction<'_,Postgres>,id:Uuid)->Result<Option<ObligationPaymentRecord>,Wave2RepositoryError>{sqlx::query_as::<_,ObligationPaymentRecord>("SELECT id,business_id,organization_id,obligation_id,idempotency_key,finance_entry_id,paid_amount,paid_on,due_on_before,due_on_after,created_by_user_id,created_at FROM business_obligation_payments WHERE id=$1").bind(id).fetch_optional(&mut **tx).await.map_err(Into::into)}
async fn load_purchase(pool:&PgPool,business_id:Uuid,key:Uuid)->Result<Option<PurchaseRecord>,Wave2RepositoryError>{sqlx::query_as::<_,PurchaseRecord>("SELECT id,business_id,organization_id,ingredient_id,idempotency_key,stock_quantity_delta,total_amount,account_key,occurred_on,note,finance_entry_id,created_by_user_id,created_at FROM business_purchases WHERE business_id=$1 AND idempotency_key=$2").bind(business_id).bind(key).fetch_optional(pool).await.map_err(Into::into)}
async fn load_purchase_tx(tx:&mut Transaction<'_,Postgres>,id:Uuid)->Result<Option<PurchaseRecord>,Wave2RepositoryError>{sqlx::query_as::<_,PurchaseRecord>("SELECT id,business_id,organization_id,ingredient_id,idempotency_key,stock_quantity_delta,total_amount,account_key,occurred_on,note,finance_entry_id,created_by_user_id,created_at FROM business_purchases WHERE id=$1").bind(id).fetch_optional(&mut **tx).await.map_err(Into::into)}

#[cfg(test)]
mod tests{
 use super::*;
 #[test]fn finance_plan_cannot_allocate_more_than_available_cash(){let request=FinancePlanRequest{owner_payroll_bps:2500,staff_payroll_bps:2500,working_capital_bps:2500,operations_bps:2000,reserve_bps:1000};assert!(matches!(validate_finance_plan(&request),Err(Wave2RepositoryError::Validation("finance_allocation_exceeds_100_percent"))));}
 #[test]fn recurring_obligation_cannot_be_income(){assert!(canonical_expense_type("other_income").is_err());assert_eq!(canonical_expense_type("utilities").unwrap(),"utilities_expense");}
}
