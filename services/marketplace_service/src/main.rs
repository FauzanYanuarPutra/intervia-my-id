...
async fn report_content(
    ...
    if content.content_type.eq_ignore_ascii_case("news") {
        return err(StatusCode::CONFLICT, "news reports must use the editorial report flow")
            .into_response();
    }
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to submit report").into_response();
        }
    };
...
async fn list_moderation_queue(
    ...
    let status = query.get("status").map(|value| value.trim()).filter(|value| !value.is_empty());
    let severity = query.get("severity").map(|value| value.trim()).filter(|value| !value.is_empty());
    let limit = query
        .get("limit")
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to load moderation queue")
                .into_response();
        }
    };
...
async fn get_moderation_history(
    ...
    let content = match find_content(&state.db, &content_id.to_string()).await {
        Ok(Some(value)) => value,
        Ok(None) => return err(StatusCode::NOT_FOUND, "content not found").into_response(),
        Err(_) => return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to load content").into_response(),
    };
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to load moderation history")
                .into_response();
        }
    };
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to load moderation events")
                .into_response();
        }
    };
    ...
    let case_items: Vec<Value> = cases.into_iter().map(|row| {
        json!({
            "id": row.get::<Uuid,_>("id"),
    ...
    let appeal_items: Vec<Value> = appeals.into_iter().map(|row| {
        json!({
            "id": row.get::<Uuid,_>("id"),
            "case_id": row.get::<Uuid,_>("case_id"),
...
async fn moderate_content(
    ...
    let action = match normalize_moderation_action(&payload.action) {
        Some(value) => value,
        None => return err(StatusCode::BAD_REQUEST, "unsupported moderation action").into_response(),
    };
    let reason_code = match normalize_moderation_reason(&payload.reason_code) {
        Some(value) => value,
        None => return err(StatusCode::BAD_REQUEST, "unsupported moderation reason").into_response(),
    };
    let reason_note = payload.reason_note
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(|value| value.chars().take(4000).collect::<String>());
    ...
    if moderation_action_requires_note(action) && reason_note.is_none() {
        return err(StatusCode::BAD_REQUEST, "reason note is required for this moderation action").into_response();
    }
    if reason_code == "other" && reason_note.is_none() {
        return err(StatusCode::BAD_REQUEST, "reason note is required when reason_code is other").into_response();
    }
    ...
    if existing.content_type.eq_ignore_ascii_case("news") {
        return err(StatusCode::CONFLICT, "news must be moderated through CMS editorial workflow")
            .into_response();
    }

    if action == "restore" && !has_content_admin_access(&claims) {
        return err(StatusCode::FORBIDDEN, "only admin can restore restricted content").into_response();
    }

    let severity = normalize_moderation_severity(payload.severity.as_deref());
    let legal_hold = payload.legal_hold.unwrap_or(matches!(severity, "high" | "critical"));
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to create moderation case").into_response();
        }
    };
    let case_id: Uuid = case_row.get("id");
    ...
    if moderation_action_changes_visibility(action) {
        ...
        match update {
            ...
            Err(error) => {
                ...
                return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to apply moderation decision").into_response();
            }
        }
    ...
    let next_case_status = if action == "escalate" { "escalated" } else { "resolved" };
    ...
    {
        let _ = tx.rollback().await;
        return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to finalize moderation case").into_response();
    }
    ...
    {
        let _ = tx.rollback().await;
        return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to write moderation evidence").into_response();
    }

    if tx.commit().await.is_err() {
        return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to commit moderation decision").into_response();
    }
...
async fn submit_content_appeal(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(id): Path<String>,
    ...
    {
        Ok(Some(value)) => value,
        Ok(None) => return err(StatusCode::CONFLICT, "no moderation decision available for appeal").into_response(),
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to submit appeal").into_response();
        }
    };
...
async fn review_content_appeal(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    ...
    if appeal.get::<Option<Uuid>,_>("last_reviewer_id") == Some(reviewer_id) {
        return err(StatusCode::CONFLICT, "appeal must be reviewed by a different person").into_response();
    }
    ...
    let next_status = if action == "overturned" { "active" } else { previous_status.as_str() };
    ...
    if action == "overturned" {
        ...
        {
            let _ = tx.rollback().await;
            return err(StatusCode::INTERNAL_SERVER_ERROR, "failed to restore content").into_response();
        }
    }
    ...
    let case_status = if action == "needs_information" { "appealed" } else { "resolved" };
    if sqlx::query(
...
async fn main() -> anyhow::Result<()> {
    ...
    if migrate_only {
        tracing::info!("marketplace migration-only release step completed");
        return Ok(());
    }
    ...
    let app = Router::new()
        ...
        .route("/v1/content/moderation/queue", get(list_moderation_queue))
        .route(
            "/v1/content/{id}",
            get(get_content)
                .put(update_content)
        ...
        .route("/v1/content/{id}/report", post(report_content))
        .route("/v1/content/{id}/moderate", post(moderate_content))
        .route("/v1/content/{id}/moderation/history", get(get_moderation_history))
        .route("/v1/content/{id}/appeal", post(submit_content_appeal))
        .route("/v1/content/appeals/{appeal_id}/review", post(review_content_appeal))
    ...
    tracing::info!(address = %addr, "marketplace service listening");
