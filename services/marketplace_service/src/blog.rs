...
fn normalize_language(value: Option<String>) -> Result<String, &'static str> {
    let language = clean(value)
        .unwrap_or_else(|| "id".to_string())
        .to_lowercase();
    if language == "id" || language == "en" {
        Ok(language)
    } else {
        Err("language must be id or en")
    }
}
...
fn normalize_category(value: Option<String>) -> Result<String, &'static str> {
    ...
    if category.len() > MAX_CATEGORY_LEN {
        return Err("category is too long");
    }
...
fn normalize_topics(value: Option<Vec<String>>) -> Result<Vec<String>, &'static str> {
    ...
    for raw in value.unwrap_or_default() {
        ...
        if topic.is_empty() || topic.len() > MAX_TOPIC_LEN || !seen.insert(topic.clone()) {
            continue;
        }
        result.push(topic);
        if result.len() >= 8 {
...
fn editorial_status(row: &BlogRow) -> String {
    row.metadata
        .get("blog")
        .and_then(|v| v.get("editorial_status"))
        .and_then(Value::as_str)
        .unwrap_or(if row.content_status == "active" {
            "published"
        } else {
            "draft"
        })
...
async fn list_blog(
    ...
    if !(0..=MAX_PUBLIC_OFFSET).contains(&offset) {
        return error(
    ...
    if language.as_deref().is_some_and(|v| v != "id" && v != "en") {
        return error(StatusCode::BAD_REQUEST, "language must be id or en").into_response();
    }
    let q = clean(query.q);
...
async fn get_blog(
    ...
    match item {
        ...
        Ok(None) => error(StatusCode::NOT_FOUND, "blog article not found").into_response(),
        Err(e) => {
...
async fn create_submission(
    ...
    let owner_id = match user_id_from_auth(&headers, &state.jwt_secret) {
        Some(id) => id,
        None => return error(StatusCode::UNAUTHORIZED, "unauthorized").into_response(),
    };
    let title = payload.title.trim().to_string();
    let summary = payload.summary.trim().to_string();
    let body = payload.body.trim().to_string();
    if title.len() < 10 || title.len() > MAX_TITLE_LEN {
        return error(StatusCode::BAD_REQUEST, "title must be 10-180 characters").into_response();
    }
    if summary.len() < 20 || summary.len() > MAX_SUMMARY_LEN {
        return error(
            StatusCode::BAD_REQUEST,
    ...
    if body.len() < 120 || body.len() > MAX_BODY_LEN {
        return error(StatusCode::BAD_REQUEST, "body must be 120-20000 characters").into_response();
    }
    let category = match normalize_category(payload.category) {
        Ok(v) => v,
        Err(m) => return error(StatusCode::BAD_REQUEST, m).into_response(),
    };
    let language = match normalize_language(payload.language) {
        Ok(v) => v,
        Err(m) => return error(StatusCode::BAD_REQUEST, m).into_response(),
    };
    let topics = match normalize_topics(payload.topics) {
        Ok(v) => v,
        Err(m) => return error(StatusCode::BAD_REQUEST, m).into_response(),
    };
    let publication_mode = clean(payload.publication_mode)
        .unwrap_or_else(|| "review".to_string())
        .to_lowercase();
    if publication_mode != "review" && publication_mode != "instant" {
        return error(
            StatusCode::BAD_REQUEST,
            "publication_mode must be review or instant",
        )
        .into_response();
    }
    let can_instant = auth_claims_from_headers(&headers, &state.jwt_secret)
        .is_some_and(|claims| has_cms_access(&claims));
    if publication_mode == "instant" && !can_instant {
        return error(
            StatusCode::FORBIDDEN,
            "instant publication requires editorial access",
        )
        .into_response();
    }
    let author_name = clean(payload.author_name)
        .unwrap_or_else(|| "Lajukan Community".to_string())
        .chars()
        .take(MAX_AUTHOR_LEN)
        .collect::<String>();
    let rich_body = sanitize_rich_body(
        payload
            .rich_body
            .as_deref()
            .unwrap_or(&format!("<p>{}</p>", body)),
    );
    if rich_body.len() > MAX_RICH_BODY_LEN {
        return error(StatusCode::BAD_REQUEST, "rich_body is too long").into_response();
    }
    ...
    if duplicate {
        return error(StatusCode::CONFLICT, "duplicate blog submission").into_response();
    }
    let slug = match unique_slug(&state.db, &title).await {
        Ok(v) => v,
        Err(e) => {
            tracing::error!("blog slug error: {:?}", e);
            return error(StatusCode::INTERNAL_SERVER_ERROR, "failed to generate slug")
                .into_response();
        }
    };
    let is_instant = publication_mode == "instant";
    let content_status = if is_instant { "active" } else { "draft" };
    let status = if is_instant {
        "published"
    } else {
        "pending_review"
    };
    let metadata = metadata_blog(
        &category,
        &language,
        &topics,
        &author_name,
    ...
    let mut tags = vec!["blog".to_string(), category.to_lowercase()];
    ...
    match inserted {
        Ok(inserted) => {
            let _=sqlx::query("INSERT INTO blog_editorial_events(content_id,actor_id,actor_role,action,to_status) VALUES($1,$2,'contributor',$3,$4)").bind(inserted.id).bind(owner_id).bind(if is_instant{"auto_publish"}else{"submit_review"}).bind(status).execute(&state.db).await;
            (StatusCode::CREATED, Json(inserted)).into_response()
        }
        Err(e) => {
            tracing::error!("blog insert error: {:?}", e);
            error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "failed to create blog article",
            )
            .into_response()
        }
    }
...
async fn update_submission(
    ...
    let content_id = match Uuid::parse_str(id.trim()) {
        Ok(v) => v,
        Err(_) => return error(StatusCode::BAD_REQUEST, "invalid article id").into_response(),
    };
    ...
    if current.owner_id != owner_id {
        return error(StatusCode::FORBIDDEN, "forbidden").into_response();
    }
    if editorial_status(&current) == "published" {
        return error(
            StatusCode::CONFLICT,
            "published blog cannot be edited as a pending submission",
        )
        .into_response();
    }
    let title = payload.title.trim().to_string();
    ...
    {
        return error(StatusCode::BAD_REQUEST, "invalid article fields").into_response();
    }
    let category = match normalize_category(payload.category) {
        Ok(v) => v,
        Err(m) => return error(StatusCode::BAD_REQUEST, m).into_response(),
    ...
    let is_instant = publication_mode == "instant";
    let next_status = if is_instant {
        "published"
    } else {
        "pending_review"
    };
    let content_status = if is_instant { "active" } else { "draft" };
    let meta = metadata_blog(
        &category,
        &language,
        &topics,
        &author_name,
    ...
    let slug = match unique_slug(&state.db, &title).await {
        Ok(v) => v,
        Err(_) => current.slug.clone().unwrap_or_else(|| slug_base(&title)),
    };
    ...
    match updated {
        Ok(updated) => {
            let _=sqlx::query("INSERT INTO blog_editorial_events(content_id,actor_id,actor_role,action,from_status,to_status) VALUES($1,$2,'contributor','resubmit',$3,$4)").bind(content_id).bind(owner_id).bind(editorial_status(&current)).bind(next_status).execute(&state.db).await;
            (StatusCode::OK, Json(updated)).into_response()
        }
        Err(e) => {
            tracing::error!("blog update error: {:?}", e);
            error(
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn editorial_queue(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Query(query): Query<std::collections::HashMap<String, String>>,
) -> impl IntoResponse {
    let claims = match auth_claims_from_headers(&headers, &state.jwt_secret) {
        Some(v) if has_cms_access(&v) => v,
        _ => return error(StatusCode::FORBIDDEN, "cms access required").into_response(),
    };
    let _ = claims;
    ...
    if !valid.contains(&requested_status.as_str()) {
        return error(StatusCode::BAD_REQUEST, "invalid editorial status").into_response();
    }
    ...
    match items {
        Ok(items) => (StatusCode::OK, Json(EditorialResponse { items })).into_response(),
        Err(e) => {
            tracing::error!("blog editorial queue error: {:?}", e);
            error(
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn moderate(
    ...
    let (next_status, content_status) = match action.as_str() {
        "approve" | "correct" => ("published", "active"),
        "needs_revision" => ("needs_revision", "draft"),
        "reject" => ("rejected", "archived"),
        "retract" => ("retracted", "archived"),
        _ => return error(StatusCode::BAD_REQUEST, "unsupported editorial action").into_response(),
    };
    if action != "approve"
        && action != "correct"
        && payload.note.as_deref().unwrap_or("").trim().is_empty()
    {
        return error(StatusCode::BAD_REQUEST, "note is required").into_response();
    }
    ...
    let blog = blog.as_object_mut().unwrap();
    blog.insert("editorial_status".into(), Value::String(next_status.into()));
    blog.insert(
        "is_indexable".into(),
        Value::Bool(next_status == "published"),
    );
    blog.insert("updated_at".into(), Value::String(Utc::now().to_rfc3339()));
    if let Some(note) = payload
        ...
        .filter(|v| !v.is_empty())
    {
        blog.insert("editor_note".into(), Value::String(note.into()));
    }
    let published_at = if next_status == "published" {
        Some(Utc::now())
    } else {
        None
    };
    ...
    match updated {
        Ok(updated) => {
            let _=sqlx::query("INSERT INTO blog_editorial_events(content_id,actor_id,actor_role,action,from_status,to_status,note) VALUES($1,$2,'editor',$3,$4,$5,$6)").bind(content_id).bind(reviewer_id).bind(&action).bind(current_status).bind(next_status).bind(payload.note).execute(&state.db).await;
            (StatusCode::OK, Json(updated)).into_response()
        }
        Err(e) => {
            tracing::error!("blog moderation error: {:?}", e);
            error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "failed to moderate article",
            )
            .into_response()
        }
    }
