          ...
          AND (
            ...
            OR (
              ...
              AND (
                ...
                OR (
                  regexp_replace($1, '[^0-9]', '', 'g') LIKE '0%'
                  AND regexp_replace(COALESCE(u.phone, ''), '[^0-9]', '', 'g')
                    ILIKE '%' || ('62' || substring(regexp_replace($1, '[^0-9]', '', 'g') FROM 2)) || '%'
                )
                OR (
                  regexp_replace($1, '[^0-9]', '', 'g') LIKE '62%'
                  AND regexp_replace(COALESCE(u.phone, ''), '[^0-9]', '', 'g')
                    ILIKE '%' || ('0' || substring(regexp_replace($1, '[^0-9]', '', 'g') FROM 3)) || '%'
                )
              )
            )
          )
