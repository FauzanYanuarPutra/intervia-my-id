...
ALTER TABLE core.users DROP CONSTRAINT IF EXISTS users_phone_format;
ALTER TABLE core.users
ADD CONSTRAINT users_phone_format CHECK (
        phone IS NULL
        OR regexp_replace(phone, '[^0-9]', '', 'g') ~ '^[0-9]{8,15}$'
    );
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_phone_normalized_unique ON core.users ((regexp_replace(phone, '[^0-9]', '', 'g')))
WHERE deleted_at IS NULL
...
SET phone = seed.phone,
    ...
    email = seed.email,
    email_verified = CASE
        WHEN seed.email IS NOT NULL THEN TRUE
        ELSE FALSE
    END,
    password_hash = CASE
        WHEN seed.keep_password THEN u.password_hash
        ELSE NULL
    END,
