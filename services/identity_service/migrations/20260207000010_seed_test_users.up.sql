...
-- END;
-- IF TG_OP = 'DELETE' THEN row_old := to_jsonb(OLD);
-- payload := jsonb_build_object('old', row_old);
-- ELSIF TG_OP = 'UPDATE' THEN row_new := to_jsonb(NEW);
-- row_old := to_jsonb(OLD);
-- payload := jsonb_build_object('old', row_old, 'new', row_new);
-- ELSE row_new := to_jsonb(NEW);
...
--     row_old->>'user_id'
-- );
-- IF target_user_text IS NOT NULL THEN BEGIN target_user := target_user_text::uuid;
-- EXCEPTION
...
--     deleted_at = NULL,
--     updated_at = NOW();
-- -- Seed display names in core.user_profiles (full_name is on user_profiles, not users)
...
--     id
-- FROM roles
-- WHERE name = 'super_admin' ON CONFLICT DO NOTHING;
-- INSERT INTO core.user_roles (user_id, role_id)
-- SELECT u.id::uuid,
--     r.id
-- FROM (
